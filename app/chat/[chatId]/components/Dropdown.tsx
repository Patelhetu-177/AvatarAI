...
export const Trigger: React.FC<TriggerProps> = ({ type, children, className, id, href }) => {
  ...
  return (
    <>
      {type === 'a' ? (
        <Link id={id} href={href || "/#"} onClick={(e: React.MouseEvent<HTMLAnchorElement>) => { // <--- CHANGED: Use href and React.MouseEvent
          e.preventDefault();
          toggleOpen();
...
const Content: React.FC<ContentProps> = ({ as: Component = 'div', className, children, placement }) => {
  ...
  const isRtl = typeof document !== 'undefined' && document.getElementsByTagName("html")[0].getAttribute("dir");
  ...
  return (
    ...
    >
      {() => ( // No need for 'status' if not using it
        <Component
