...
function validateAndPrepareEmbedding(embedding: unknown): number[] {
  ...
  if (validEmbedding.length !== EMBEDDING_DIMENSION) {
    console.warn(
      `Embedding dimension (${validEmbedding.length}) does not match expected (${EMBEDDING_DIMENSION}). Adjusting...`,
    );
  ...
  try {
    ...
    if (Array.isArray(embedding)) {
      if (embedding.length > 0 && typeof embedding[0] === "number") {
        ...
        embeddingArray = (embedding as string[])
          .map(Number)
          .filter((n) => !isNaN(n));
      }
    ...
    console.error("Error in storeQuestionEmbedding:", {
      ...
      embeddingType: typeof embedding,
      embeddingSample: Array.isArray(embedding)
        ? embedding.slice(0, 3)
        : embedding,
    });
    return false;
